defmodule Challenge.UserTransactionServerTest do
  ...
  test "bet/2 returns error for insufficient funds, RS_ERROR_NOT_ENOUGH_MONEY(7)", %{
    server: server,
    user_id: user_id
  } do
  ...
  test "win/2 returns error for missing reference_transaction_uuid, RS_ERROR_TRANSACTION_DOES_NOT_EXIST(14)",
    ...
    assert %{status: "RS_ERROR_TRANSACTION_DOES_NOT_EXIST"} =
             Challenge.UserTransactionServer.win(server, win_params)
  end

  test "bet/2 returns error for wrong currency, RS_ERROR_WRONG_CURRENCY(6)", %{
    server: server,
    user_id: user_id
  } do
  ...
  test "bet/2 returns error for disabled user, RS_ERROR_USER_DISABLED(8)", %{
    server: server,
    user_id: user_id
  } do
  ...
  test "bet/2 returns error for duplicate transaction with mismatched params, RS_ERROR_DUPLICATE_TRANSACTION(13)",
       %{server: server, user_id: user_id} do
